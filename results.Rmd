...
top_lol$sum <- as.numeric(top_lol$sum)
top_lol$Label <- factor(top_lol$Label, levels = top_lol$Label[order(top_lol$sum)])



top_lol$ID <- reorder(top_lol$ID, top_lol$sum)
...
p_top_lol <- top_lol %>%
  mutate(Label=as.factor(Label),
    Label=fct_reorder(Label, sum, .desc=TRUE)) %>%
  ggplot( aes(x=sum, y=Label)) +
  scale_x_log10() +
  geom_point( aes(colour=Habitat)) +        
  geom_segment( aes(x=0, xend=sum, y=Label, yend=Label, colour=Habitat)) +
