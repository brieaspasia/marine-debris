...
p_top_items <- 
  ggplot(top_items, aes(x=sum, y=item)) +
  geom_point() +        
...
p_hab_top_items <- 
  ggplot(hab_top_items, aes(x=sum, y=item, colour=Material)) +
  geom_point() +        
  geom_segment( aes(x=0, xend=sum, y=item, yend=item))
...
top_lol$ID <- as.factor(top_lol$ID)
...
top_lol <- top_lol %>%
  filter(ID %in% top20) %>%
  group_by(Habitat, ID, item_name) %>%
  summarise(sum=sum(Total)) 

HabitatLabs <- c(P = "Pier", SS = "Sediment")

p_top_lol <- top_lol %>%
  mutate(ID=fct_reorder(ID, sum)) %>%
  ggplot( aes(x=sum, y=item_name)) +
  geom_point( aes(colour=Habitat)) +        
  geom_segment( aes(x=0, xend=sum, y=item_name, yend=item_name, colour=Habitat))
