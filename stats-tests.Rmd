...
```{r glm-2}
#create an ID for each survey event as a factor and use that for AR1.

#create df to use for models
mod <- site_select %>%
  group_by(Date, Location, Habitat, DistKm, NumberSurveys, Fishing) %>% #relevant variables
...
```
```{r testing-temporal-autocorrelation}
...
#why is this missing 'Encumbered drift'?  And what does this mean?
library(lattice)
