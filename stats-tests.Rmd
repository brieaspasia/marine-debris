...
AIC(m_hab1, m_hab2, m_hab3, m_hab4, m_hab5, m_hab6, m_hab7) #but AIC shows less than 2 pts difference with or without coast so choose the model with less terms
...
nd <- expand.grid(Habitat = unique(mod_hab$Habitat),
...
```{r}
nd$p <- predict.glmmTMB(object = m_hab,
                        newdata = nd,
                        se.fit = F,
                        re.form = NA,
                        type="response")
...
3. plot the predicted means. You can add standard errors too but just see if this works first.


```{r}
ggplot(nd, aes(y=Total, x=DistKm)) + 
