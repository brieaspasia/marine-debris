...
#TR would be ID type as row, columns would be either fishing vs non-fishing and either heavy, lgt, or entangling
#Env would be habitat and distKM
#abund would be ID as columns and survey events as rows
```
...
abund <- spread(abund_test, ID, sum) #convert to data wide

#create env df with habitat and distKm
env <- antTraits$env

#create survey event as row name, then habitat and distance
env <- site_select %>%
  mutate(survey_event=paste0(Date, Site)) %>% #create column for unique survey event
  dplyr::select(survey_event, Habitat, DistKm) %>%
  distinct()

#convert survey event to rownames
rownames_to_column(env, var="survey_event")
