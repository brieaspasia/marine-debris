...
drop1(m_temp, test="Chisq") #Fishing: Habitat:DistKm  1 257.93 6.6198  0.01008 *
...
site_select$Material[site_select$Material %in% var_change] = "Glass" 
...
#create vector from top 5 items
top_mat <- top_mat$Material
top_mat <- unlist(top_mat)
...
drop1_interaction = purrr::map_df(txt, function(x){
  
  temp_df <- site_select %>%
  filter(Material == paste0(x)) %>%
  group_by(Date, Location, Habitat, DistKm) %>% #relevant variables
  summarise(Total=sum(Total)) %>%
  ungroup() %>%
  mutate(Location = as.factor(Location),
...
drop1_main = purrr::map_df(txt, function(x){
