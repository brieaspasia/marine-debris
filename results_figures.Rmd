...
```{r glm-nonfish-drop, warning=FALSE, message=FALSE}
#create df to use for models
mod_nonfish_drop <- site_select %>%
  filter(Fishing=="Non-Fishing") %>% #nonfishing debris
  group_by(Date, Location, Habitat, DistKm) %>% #relevant variables
  summarise(Total=sum(Total)) %>%
  ungroup() %>%
...
m_nonfish_drop  <- glmmTMB(Total ~ #Debris count
                 Habitat + #predicted by Habitat
                 (1|Location) + (1|Date), #site random effect
               family=nbinom1(), #negative binomial to deal with count data and zeros
               data = mod_nonfish_drop)

# Predictors for non-fishing related debris are: Habitat

nd_nonfish_drop <- expand.grid(Habitat = unique(mod_nonfish_drop$Habitat),
                          # DistKm = seq(from = min(mod_nonfish$DistKm),
                          #      to = max(mod_nonfish$DistKm),
                          #      length=1000),
                  Location = NA,
...
pred_nonfish_drop <- predict(object = m_nonfish_drop,
                         newdata = nd_nonfish_drop,
                         se.fit = T,
                         re.form = NA,
                         type="response")

nd_nonfish_drop$Total <- pred_nonfish_drop$fit
nd_nonfish_drop$SE_upper <- pred_nonfish_drop$fit + pred_nonfish_drop$se.fit
nd_nonfish_drop$SE_lower <- pred_nonfish_drop$fit - pred_nonfish_drop$se.fit


p_nonfish_drop <- ggplot(nd_nonfish_drop, aes(y=Total, x=Habitat)) + 
...
svg("images/p_nonfish_drop.svg")
plot(p_nonfish_drop, height=7, width=7, units="in")
dev.off()
...
```{r glm-btl_drop, warning=FALSE, message=FALSE}
...
#GLM negative binomial with two variables and zero inflation
m_btl_drop  <- glmmTMB(Total ~ #Debris count
                 Habitat + #predicted by Habitat and distance
...
nd_btl_drop <- expand.grid(Habitat = unique(mod_btl$Habitat),
                  Location = NA,
                  Date = NA)

pred_btl_drop <- predict(object = m_btl_drop,
                         newdata = nd_btl_drop,
                         se.fit = T,
                         re.form = NA,
                         type="response")

nd_btl_drop$Total <- pred_btl_drop$fit
nd_btl_drop$SE_upper <- pred_btl_drop$fit + pred_btl_drop$se.fit
nd_btl_drop$SE_lower <- pred_btl_drop$fit - pred_btl_drop$se.fit

#plot the predicted means

p_btl_drop <- ggplot(nd_btl_drop, aes(y=Total, x=Habitat)) +
  geom_col(aes(fill=Habitat)) +
  geom_errorbar(aes(ymin=SE_lower, ymax=SE_upper, width=0.2), position=position_dodge(width=0.90)) 
   # theme(legend.position = "none",
   #      panel.grid = element_blank(),
...
# ggsave(filename = "images/p_btl_drop.svg", plot = p_btl_drop, device="svg", height=7, width=7, units="in")

# svg("images/p_btl_drop.svg")
# plot(p_btl_drop, height=7, width=7, units="in")
# dev.off()
...
p_btl <- ggplot(nd_btl, aes(y=Total, x=DistKm)) +
  geom_line(aes(col=Habitat)) +
  geom_ribbon(aes(ymin=SE_lower, ymax=SE_upper, fill=Habitat), alpha=0.2)
   # theme(legend.position = "none",
   #      panel.grid = element_blank(),
...
```{r glm-glsmat_drop, warning=FALSE, message=FALSE}
#create df of all misc materials
mod_gls <- site_select %>%
  filter(Material=="Glass") %>% #glass
  group_by(Date, Location, Habitat, DistKm) %>% #relevant variables
...
m_gls_drop  <- glmmTMB(Total ~ #Debris count
                 Habitat + #predicted by Habitat
                 (1|Location) + (1|Date), #site/date random effect
               family=nbinom1(), #negative binomial to deal with count data and zeros
               data = mod_gls)
...
nd_gls_drop <- expand.grid(Habitat = unique(mod_gls$Habitat),
                  Location = NA,
                  Date = NA)

pred_gls_drop <- predict(object = m_gls_drop,
                         newdata = nd_gls_drop,
                         se.fit = T,
                         re.form = NA,
                         type="response")

nd_gls_drop$Total <- pred_gls_drop$fit
nd_gls_drop$SE_upper <- pred_gls_drop$fit + pred_gls_drop$se.fit
nd_gls_drop$SE_lower <- pred_gls_drop$fit - pred_gls_drop$se.fit

#plot the predicted means

p_gls_drop <- ggplot(nd_gls_drop, aes(y=Total, x=Habitat)) +
  geom_col(aes(fill=Habitat)) +
  geom_errorbar(aes(ymin=SE_lower, ymax=SE_upper, width=0.2), position=position_dodge(width=0.90)) +
  ylim(0, 30) +
   theme(legend.position = "none",
...
# ggsave(filename = "images/p_gls_drop.svg", plot = p_gls_drop, device="svg", height=7, width=7, units="in")

svg("images/p_gls_drop.svg")
plot(p_gls_drop, height=7, width=7, units="in")
dev.off()
...
# #GLM negative binomial with two variables and zero inflation
m_met_drop  <- glmmTMB(Total ~ #Debris count
                 Habitat + #predicted by Habitat
                 (1|Location) + (1|Date), #site/date random effect
               family=nbinom1(), #negative binomial to deal with count data and zeros
               data = mod_met)
...
nd_met_drop <- expand.grid(Habitat = unique(mod_met$Habitat),
                  Location = NA,
                  Date = NA)

pred_met_drop <- predict(object = m_met_drop,
                         newdata = nd_met_drop,
                         se.fit = T,
                         re.form = NA,
                         type="response")

nd_met_drop$Total <- pred_met_drop$fit
nd_met_drop$SE_upper <- pred_met_drop$fit + pred_met_drop$se.fit
nd_met_drop$SE_lower <- pred_met_drop$fit - pred_met_drop$se.fit

#plot the predicted means

p_met_drop <- ggplot(nd_met_drop, aes(y=Total, x=Habitat)) +
  geom_col(aes(fill=Habitat)) +
  geom_errorbar(aes(ymin=SE_lower, ymax=SE_upper, width=0.2), position=position_dodge(width=0.90)) +
  ylim(0, 30) 
   # theme(legend.position = "none",
...
# svg("images/p_met_drop.svg")
# plot(p_met_drop, height=7, width=7, units="in")
# dev.off()

# ggsave(filename = "images/p_met_drop.svg", plot = p_met_drop, device="svg", height=7, width=7, units="in")
```
...
```{r gllvm-set-up-1or2, warning=FALSE, message=FALSE, results="hide"}
#Abundance
  #create abundance dataframe with ID as columns and survey events as rows
...
#Environment
  #create survey event and filter zero sum surveys, 
    ...
    mutate(survey_event=paste0(Date, Site)) %>%
      ...
      distinct() %>%
      ungroup() %>%
      mutate(hab = ifelse(Habitat=="P", 1, 0))
  ...
  env$hab <- as.numeric(env$hab) #convert factor to numeric

  env <- env %>%
    dplyr::select(DistKm, hab)
...
fit4th <- gllvm(y, X, TR, family="negative.binomial",
                formula= y~(hab + DistKm) +
                  (hab + DistKm) : (Dispersiveness + Fishing + Material))
...
summary(fourth)
```
```{r gllvm-set-up-separateHab-contTraits, warning=FALSE, message=FALSE, results="hide"}
#Abundance
  #create abundance dataframe with ID as columns and survey events as rows
  abund_long <- site_select %>%
    mutate(survey_event=paste0(Date, Site)) %>% #create column for unique survey event
...
#Traits
  ...
  # TR$Dispersiveness <- as.factor(TR$Dispersiveness) #convert character to factor with 3 levels
  TR$Fishing <- as.factor(TR$Fishing) #convert character to factor with 2 levels
  TR$Material <- as.factor(TR$Material) #convert character to factor with 4 levels
