...
lib.load = c("here", "ggplot2", "dplyr", "lubridate", "forcats", "ggthemes", "knitr", "kableExtra", "stats", "broom") 
...
item_total_per_habitat <- left_join(item_total_per_habitat, item_code, by="ID") #join item names

top_items_per_pier <- item_total_per_habitat %>% 
  filter(Habitat=="P") %>%
  top_n(10, sum) #show top 20 items over all pier counts

top_items_per_ss <- item_total_per_habitat %>% 
  filter(Habitat=="SS") %>%
  top_n(10, sum) #show top 10 items over all ss counts

top_items_per_habitat <- rbind(top_items_per_pier, top_items_per_ss)
    
kable(top_items_per_habitat, booktabs = TRUE) %>%
  pack_rows(index=c("P" = 10, "SS"=10))

rmarkdown::paged_table(top_items_per_habitat) %>% #print clean table
kable("html", 
      booktabs=TRUE,
...
ggscatter(mds, x = "Dim.1", y = "Dim.2", 
          label = rownames(top_items_per_habitat),
          size = 1,
          repel = TRUE)
```
...
fishing <- site_select %>%
  ...
  summarise(item_total=sum(Total), #total fishing items
            mean=item_total/unique(NumberSurveys),
...
p_habitat <- habitat %>%
  ...
  geom_bar(position="dodge", stat="identity") +
  geom_errorbar(aes(ymin=mean-mean_sd, ymax=mean+mean_sd, width=0.2), position=position_dodge(width=0.90)) +
...
```{r scatterplot}
scatter <- site_select
  scatter$Material <- as.character(scatter$Material) #change Material to character
scatter$Material[scatter$Material %in% var_change] = "Other" #set categories to Other

scatter %>%
  filter(Total>0) %>%
  ggplot( aes(ID, Total, colour=Material)) +
  geom_point(position="jitter", alpha=0.5) +
  coord_trans(y="log10") +
  facet_wrap(~Habitat)

  #normalise sites
site_total_count <- site_select %>%
  group_by(Habitat, Location) %>%
  mutate(mean=sum(Total)/NumberSurveys, #mean of item type per site
         mean_sd=sd(Total)/NumberSurveys, #sd of item type per site
         sum=sum(Total),
         sd=sd(Total)) %>% 
        ungroup()

site_total_count <- site_total_count %>%
  select(Location, Habitat, sum, sd, mean, mean_sd) %>%
  distinct()

ggplot(site_total_count, aes(mean, sum, colour=Habitat)) +
  geom_point()

```
```{r glm}
#Fit a model to estimate an intercept and the difference btwn P vs SS
...
tidy(m1)
coef(m1)
confint(m1) #shouldn't be negative?

plot(m1)

# #Fit a model that estimates the intercept of both habitats
# m2 <- glm(Total ~ Habitat-1, 
#           data = site_select, 
#           family = "poisson")
# summary(m2)
# tidy(m2)
