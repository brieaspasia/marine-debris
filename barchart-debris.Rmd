...
var_change <- c("Ceramic", "Undetermined", "E-Waste", "Paper", "Construction", "Organic", "Unknown", "Timber") #state variables to rename
...
total_P <- P %>%
  ...
  summarise(item_total=sum(Total), #total number of each item per site
            mean=item_total/NumberSurveys) %>% #mean of each item per site
  arrange(desc(mean)) #listed by largest means

total_P <- distinct(total_P) #removing duplicate records
total_P <- left_join(total_P, item_code, by="ID") #adding item name

total_P <- total_P %>%
  filter(mean>0) #filter out null values
...
total_SS <- distinct(total_SS) #removing duplicate records
...
fishing <- site_select %>%
  ...
  summarise(item_total=sum(Total), #total fishing items
            mean=item_total/NumberSurveys) %>% #total per survey
  arrange(desc(mean))

fishing <- distinct(fishing) #remove duplicates

site_habitat <- site_select %>%
  select(Site, Habitat, Location) %>% #create df for labeling
  distinct() #remove duplicates
...
habitat <- site_select %>%
  ...
  summarise(item_total=sum(Total), #total number of each material type per site
            mean=item_total/NumberSurveys, #mean of each material type per site
