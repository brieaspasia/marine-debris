...
```{r gllvm-set-up-only-pier, warning=FALSE, message=FALSE, results="hide"}
#Abundance
  #create abundance dataframe with ID as columns and survey events as rows
  abund_long <- site_select %>%
  filter(Habitat=="P") %>%
...
#Environment
  #create survey event and filter zero sum surveys, 
    ...
    mutate(survey_event=paste0(Date, Site)) %>%
      ...
      mutate(sum=sum(Total)) %>%
      filter(sum>0) %>%
      dplyr::select(survey_event, DistKm, Location) %>%
      distinct()
...
fit4th <- gllvm(y, X, TR, family="negative.binomial",
                formula= y~(DistKm) +
                  (DistKm) : (Dispersiveness + Fishing + Material))
...
```{r gllvm-set-up-only-ss, warning=FALSE, message=FALSE, results="hide"}
#Abundance
  #create abundance dataframe with ID as columns and survey events as rows
  abund_long <- site_select %>%
  filter(Habitat=="SS") %>%
